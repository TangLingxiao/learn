...
{
    long iTimeoutMS = options.m_iTimeoutMS;
    if (iTimeoutMS < 1000)
    {
    ...
    m_asyncHandler = new RedisAsyncHandler(shared_from_this());
    return true;
}
...
ReplyPtr Client::command(CmdArg &vdatas)
{
    ...
    int32_t retry{0};
    while (!reply && (m_ctx->err == REDIS_ERR_EOF || m_ctx->err == REDIS_ERR_IO) && retry++ < 10)
    {
        redisReconnect(m_ctx);
...
std::vector<ReplyPtr> Client::execCommand(int nums)
{
    ...
    while (nums--)
    {
        if (redisGetReply(m_ctx, &reply) != REDIS_OK)
        {
            throw std::runtime_error("get reply error");
        }
